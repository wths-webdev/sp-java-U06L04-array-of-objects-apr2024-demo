...
public class Santa {
    public static void main(String[] args) {
        ...
        for(int i =0; i < myKids.length; i++){
            ...
            // nice it set to true by default
            if(choice == 1){
        ...
        while(!done){
            ...
            // -1 to accommodate index
            int numEntered = input.nextInt() - 1;
            if(numEntered < christmasList.length && numEntered >= 0){
                // instead of getNice and then changeChild, we want to swap in one go
//                boolean nice = christmasList[numEntered].getNice();
//                hohoho.changeChild(numEntered, !nice);
                hohoho.toggleNiceOrNaughty(numEntered);
            } else {
                done = true;
            }
        }
